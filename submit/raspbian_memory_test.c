...
static void run_memory_benchmark8(int32_t reps) {
	...
	printf("\tMEMSPEED: %ld MB took %dus = %dMB/s\n",
		(uint64_t)(((uint64_t)BENCH_SIZE*(uint64_t)BENCH_ITERATIONS)/1024/1024),
		(diff),
		(uint64_t)((uint64_t)BENCH_SIZE*(uint64_t)BENCH_ITERATIONS)/(diff));

	memset_test(benchmark+OFFSET,0xfffffffe,BENCH_SIZE);
...
static void run_memory_benchmark32(int32_t reps) {
	...
	for(i=0;i<reps;i++) {
		memset_4byte(benchmark+OFFSET,0xffffffa5,BENCH_SIZE);
	}
	...
	memset_test(benchmark+OFFSET,0xffffffa5,BENCH_SIZE);
}
...
static void run_memory_benchmark_custom(int32_t reps) {
	...
	printf("\tMEMSPEED: %ld MB took %dus = %ldMB/s\n",
		(uint64_t)((((uint64_t)BENCH_SIZE*(uint64_t)BENCH_ITERATIONS)/1024/1024)),
		(diff),
		(uint64_t)((uint64_t)BENCH_SIZE*(uint64_t)BENCH_ITERATIONS)/(diff));
